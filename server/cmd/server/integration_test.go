...
func TestMain(m *testing.M) {
	...
	testPool = pool
	testUserID, testWorkspaceID, testTeamID, err = setupIntegrationTestFixture(ctx, pool)
	if err != nil {
...
func setupIntegrationTestFixture(ctx context.Context, pool *pgxpool.Pool) (string, string, string, error) {
	...
	var teamID string
	if err := pool.QueryRow(ctx, `
		INSERT INTO team (workspace_id, name, identifier, color)
		VALUES ($1, 'Default', 'INT', 'blue')
		RETURNING id
	...
	return userID, workspaceID, teamID, nil
}
